...
namespace IKVM.Internal
{
	...
	abstract class MemberWrapper
	{
		...
		protected MemberWrapper(TypeWrapper declaringType, string name, string sig, Modifiers modifiers, MemberFlags flags)
		{
            ...
            //RuntimeReflectionHelper.Instance.Mark(name);
#else
            this.name = String.Intern(name);
			this.sig = String.Intern(sig);

            //RuntimeReflectionHelper.Instance.Mark("Type = " + GetType());
            //RuntimeReflectionHelper.Instance.Mark("Name = " + this.name + " key= " + name);
            //RuntimeReflectionHelper.Instance.Mark("sig = " + this.sig + " key= " + sig);
#endif
		...
		internal string Name
		{
			get
			{
                ...
                //RuntimeReflectionHelper.Instance.Mark(method.Name);
...
#else
            throw new NotImplementedException("sdas");
#endif
#endif
        }
	...
	abstract class FieldWrapper : MemberWrapper
	{
		...
		internal static FieldWrapper FromField(java.lang.reflect.Field field)
		{
			...
			return null;
#else
            RuntimeReflectionHelper.Instance.Mark("FromField");
		...
		internal object ToField(bool copy, int? fieldIndex)
		{
			...
			return null;
#else
            RuntimeReflectionHelper.Instance.Mark("ToField started");
			...
			if (field == null)
			{
				...
				Link();
                RuntimeReflectionHelper.Instance.Mark(this.FieldTypeWrapper.EnsureLoadable(this.DeclaringType.GetClassLoader()));
            ...
            RuntimeReflectionHelper.Instance.Mark("ToField return");
            return field;
#endif // FIRST_PASS
		}
#endif // !STATIC_COMPILER && !STUB_GENERATOR
	...
	sealed class SimpleFieldWrapper : FieldWrapper
	{
		...
		{
            RuntimeReflectionHelper.Instance.Mark("SimpleFieldWrapper = " + name);
