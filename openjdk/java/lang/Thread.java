...
class Thread implements Runnable {
    ...
    private cli.IKVM.Runtime.NativeThread nativeThread;
   ...
   /**
    ...
    public static void yield() {
       ...
       cli.IKVM.Runtime.NativeThreadHelper.getInstance().Sleep(0);
    }
    ...
    Thread(ThreadGroup g) {
       ...
       // cli.System.Threading.Thread thread = cli.System.Threading.Thread.get_CurrentThread();
        nativeThread = null;
        String name = null;// thread.get_Name();
        if (name == null) {
    ...
    private static int mapClrPriorityToJava(int priority) {
        ...
        switch (priority) {
        case cli.IKVM.Runtime.NativeThreadPriority.Lowest:
            return MIN_PRIORITY;
        case cli.IKVM.Runtime.NativeThreadPriority.BelowNormal:
            return 3;
        default:
        case cli.IKVM.Runtime.NativeThreadPriority.Normal:
            return NORM_PRIORITY;
        case cli.IKVM.Runtime.NativeThreadPriority.AboveNormal:
            return 7;
        case cli.IKVM.Runtime.NativeThreadPriority.Highest:
            return MAX_PRIORITY;
        }
        //throw new Error("NotImplemented");
    }
    ...
    private static int mapJavaPriorityToClr(int priority) {
        ...
        if (priority == MIN_PRIORITY) {
            return cli.IKVM.Runtime.NativeThreadPriority.Lowest;
        }
        else if (priority > Thread.MIN_PRIORITY && priority < Thread.NORM_PRIORITY) {
            return cli.IKVM.Runtime.NativeThreadPriority.BelowNormal;
        }
        else if (priority == Thread.NORM_PRIORITY) {
            return cli.IKVM.Runtime.NativeThreadPriority.Normal;
        }
        else if (priority > Thread.NORM_PRIORITY && priority < Thread.MAX_PRIORITY) {
            return cli.IKVM.Runtime.NativeThreadPriority.AboveNormal;
        }
        else if (priority == Thread.MAX_PRIORITY) {
            return cli.IKVM.Runtime.NativeThreadPriority.Highest;
        }
        else {
            // can't happen
            return cli.IKVM.Runtime.NativeThreadPriority.Normal;
    ...
    private void start0() {
        //throw new Error("NotImplemented");
        cli.IKVM.Runtime.NativeThreadStart threadStart = new cli.IKVM.Runtime.NativeThreadStart() {
        ...
        //if (stackSize <= 0) {
         nativeThread = cli.IKVM.Runtime.NativeThreadHelper.getInstance().NewThread();
		 nativeThread.init(threadStart);
         //}
        // else {
        ...
        nativeThread.set_Priority(mapJavaPriorityToClr(priority));
    ...
    private void wakeupJoinedThreads() {
        ...
        try {
            ...
            if (locked) {
                ...
                cli.IKVM.Runtime.NativeThreadHelper.getInstance().QueueUserWorkItem(
                    new cli.IKVM.Runtime.NativeWaitCallbackWithArg() {
...
//         return m;
    ...
    public State getState() {
        ...
        if ((nativeThread.get_ThreadState() & cli.IKVM.Runtime.NativeThreadState.WaitSleepJoin) != 0) {
    ...
    private void setNativeName(String name) {
        cli.IKVM.Runtime.NativeThread thread = nativeThread;
        if (thread != null) {
            try {
